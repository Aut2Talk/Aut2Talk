...
export default class App extends React.Component {
  ...
  emojiStringToEncodedEmoji(emojiString) {
    if (typeof(emojiString) !== "string") {
      throw "Sorry! Please input a string of one character!";
    } else if (emojiString.length == 0) {
      throw "Sorry! Please enter a single character or emoji! You entered nothing!";
    } else if (emojiString.length > 2) {
      throw "Sorry! Please enter a single character or emoji! You entered too many characters.";
    } else if (emojiString.length == 2) {
      ...
      if (emojiString.charCodeAt(0) == emojiString.codePointAt(0)) {
        ...
        throw "Sorry! Please enter a single character or emoji! You entered 2 characters.";
      }
    }
  ...
  encodedEmojiToEmojiString(encodedEmoji) {
    if (typeof(encodedEmoji) !== "number") {
      throw "Sorry! Please ensure that the encodedEmoji is stored as a number." + DEBUG_MESSAGE;
    }
    if (encodedEmoji < 0 || encodedEmoji > 0x10FFFF) {
      throw "Sorry! Please ensure that the encodedEmoji is between 0 and 0x10FFF0." + DEBUG_MESSAGE;
    }
